...
if oc get itms "sail-e2e-required-images-registry-mirror" &>/dev/null && oc get idms "sail-e2e-required-images-registry-mirror" &>/dev/null; then
    echo "Skipping setup as sail-e2e-required-images-registry-mirror already exists in ITMS and IDMS"
else

oc apply -f ${SOURCE_ROOT}/jenkins-csb-declaration/resources/ocp/templates/olm/custom/mirrorSets/ibm/sail-e2e-registry-itms-idms.yaml
